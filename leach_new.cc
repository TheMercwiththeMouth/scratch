...
int runLeachSimulation_NewVisuals(const struct sensor network[], struct network_stats * new_network_stats);  
...
{  
    ...
    for(i = 0; i < TRIALS; i++){  
		// rounds_LEACH = runLeachSimulation_NewVisuals(network, &network_stats_data[i]);
		// cout<<"Running Leach Simulation with Visualization" <<endl;
...
//     mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel"); // Or another model if needed
...
// 	// Set initial positions (important for ConstantPositionMobilityModel)
//     for (int i = 0; i < NUM_NODES; ++i) { // Corrected loop condition
//         Ptr<Node> node = nodes.Get(i);
//         Ptr<ConstantPositionMobilityModel> pos = node->GetObject<ConstantPositionMobilityModel>();
//         if (!pos) {
//             pos = CreateObject<ConstantPositionMobilityModel>();
//             node->AggregateObject(pos);
//         }
//         pos->SetPosition(Vector(network_LEACH[i].xLoc, network_LEACH[i].yLoc, 0.0));
//     }
	
// 	// Base Station Node (Separate from sensor nodes)
//     ns3::NodeContainer baseStationNodeContainer;
//     baseStationNodeContainer.Create(1);
//     Ptr<Node> baseStationNode = baseStationNodeContainer.Get(0);

//     MobilityHelper bsMobility;
//     bsMobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
//     bsMobility.Install(baseStationNodeContainer);
//     Ptr<ConstantPositionMobilityModel> bsPos = baseStationNode->GetObject<ConstantPositionMobilityModel>();
//     if (!bsPos) {
//         bsPos = CreateObject<ConstantPositionMobilityModel>();
//         baseStationNode->AggregateObject(bsPos);
//     }
//     bsPos->SetPosition(Vector(BASE_STATION.xLoc, BASE_STATION.yLoc, 0.0));

// 	Ptr<Node> baseStationNode = nodes.Get(NUM_NODES);
//     Ptr<ConstantPositionMobilityModel> bsPos = baseStationNode->GetObject<ConstantPositionMobilityModel>();
...
//     }
//     bsPos->SetPosition(Vector(BASE_STATION.xLoc, BASE_STATION.yLoc, 0.0));
//     anim.UpdateNodeColor(NUM_NODES, 0, 0, 255);
	
// 	// Install Internet Stack (Necessary for routing)
//     InternetStackHelper stack;
//     stack.Install(nodes);
//     InternetStackHelper bsStack;
//     bsStack.Install(baseStationNodeContainer);

//     // Ipv4 Addressing
//     Ipv4AddressHelper address;
//     address.SetBase("10.1.1.0", "255.255.255.0");
//     Ipv4InterfaceContainer interfaces = address.Assign(nodes);
//     Ipv4InterfaceContainer bsInterface = address.Assign(baseStationNodeContainer);

//     // Routing (e.g., AODV - choose your routing protocol)
//     //  You might need to install AODV separately if it's not included by default.
//     //  In newer NS-3 versions, AODV is not included by default. You have to enable it while configuring ns3.
//     //  ./waf configure --enable-aodv
//     //  ./waf build
//     AodvHelper routing;  // Or other routing protocol
//     Ipv4GlobalRoutingHelper::PopulateRoutingTables(); // If using global routing

//     // Application (Example: OnOffApplication - adapt to your LEACH needs)
//     //  This is a simplified example.  You'll need to adapt it to your LEACH clustering.
//     //  Nodes send to cluster head, cluster head sends to base station.
//     uint16_t sinkPort = 8080; // Example port

//     // Install a sink (receiver) at the base station
//     Address sinkAddress(InetSocketAddress(bsInterface.GetAddress(0), sinkPort));
//     PacketSinkHelper packetSinkHelper("ns3::TcpSocketFactory", sinkAddress); // or UdpSocketFactory
//     ApplicationContainer sinkApps = packetSinkHelper.Install(baseStationNodeContainer);
//     sinkApps.Start(Seconds(0.));
//     sinkApps.Stop(Seconds(TOTAL_ROUNDS * 10)); // Adjust stop time as needed

//     // Example: All nodes send to the base station (adapt for LEACH)
//     OnOffHelper source("ns3::TcpSocketFactory", sinkAddress); // or UdpSocketFactory
...
//     }
//     sourceApps.Start(Seconds(1.0)); // Adjust start time as needed
//     sourceApps.Stop(Seconds(TOTAL_ROUNDS * 10)); // Adjust stop time as needed


// 	AnimationInterface anim("leach-animation.xml"); 
// 	anim.EnablePacketMetadata(true); 

// 	for (int i = 0; i < NUM_NODES; ++i) {
//         anim.UpdateNodeColor(i, 0, 255, 0); // Green for sensor nodes
//     }
//     anim.UpdateNodeColor(baseStationNode, 0, 0, 255);
...
// 				Ptr<Node> node = nodes.Get(i);
// 				if (network_LEACH[i].head == -1) {
// 					anim.UpdateNodeColor(node, 255, 0, 0); // Red for cluster heads
// 				} else if (network_LEACH[i].head == DEAD_NODE) {
// 					anim.UpdateNodeColor(node, 0, 0, 0); // Black for dead nodes
// 				} else {
// 					anim.UpdateNodeColor(node, 0, 255, 0); // Green for normal nodes
// 				}
// 			}
...
int runDirectSimulation(const struct sensor network[], struct network_stats * new_network_stats){  
	...
	network_DIRECT = new struct sensor[60];  
