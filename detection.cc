...
void faultAreaDetection(struct sensor * network, int round);
...
{  
    ...
    for(i = 0; i < TRIALS; i++){  
        ...
        printf("\n");
        printf("The modified LEACH transmission simulation was able to remain viable for %d rounds\n", rounds_LEACH_NEW);
...
//FAULT NODE RECOVERY ALGORITHMS
//Phase 1: Detect Faulty Area in a Cluster
void faultAreaDetection(struct sensor * network, int round){
    int faultNodeCount = 0;
    double totalNodes = NUM_NODES;
    ...
    if(faultPercentage >= MIN_FAULT_TOLERANCE_PERCENTAGE){
        cout << "[ALERT] Cluster " << round << " exceeds " << MIN_FAULT_TOLERANCE_PERCENTAGE << "% faulty nodes! Initiating recovery...\n";
        // InitiateRecovery();
    }
}
